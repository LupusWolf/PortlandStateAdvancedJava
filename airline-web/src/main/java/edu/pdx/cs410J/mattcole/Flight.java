...
public class Flight extends AbstractFlight implements Comparable<Flight> {
   ...
   */
  public void setFieldByEnum(Fields field, String value) throws FlightParseDateTimeException, constHolder.ArrivesBeforeDeparts, constHolder.InvalidAirportCode {
    try {
      switch (field) {
        case src:
          if (!AirportNames.getNamesMap().containsKey(value)) throw new constHolder.InvalidAirportCode();
          source = value;
          break;
        case depart:
   ...
   */
  public void SetDateFieldByEnum(Fields field, Date value) throws constHolder.ArrivesBeforeDeparts {
    switch (field)
    {
      case depart:
        departure = value;
   ...
   * @throws constHolder.ArrivesBeforeDeparts
   */
  private void updateDuration() throws constHolder.ArrivesBeforeDeparts {
    if (durationInMinutes == -1 && arrival != null && departure != null)
    {
      if (arrival.compareTo(departure) < 0)
      {
        throw new constHolder.ArrivesBeforeDeparts();
      }
  ...
  public Flight(int number, String source, String departure, String destination, String arrival) throws FlightParseDateTimeException, constHolder.ArrivesBeforeDeparts, constHolder.InvalidAirportCode {
    this.number = number;
