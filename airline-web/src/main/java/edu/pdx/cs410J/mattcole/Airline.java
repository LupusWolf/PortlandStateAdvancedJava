...
public class Airline extends AbstractAirline<Flight> {
  ...
  public void SetDetailOfFlight(int flightNumber, Flight.Fields field, String value) throws
          Flight.FlightParseDateTimeException, constHolder.ArrivesBeforeDeparts, constHolder.InvalidAirportCode {
  ...
  public Airline getSubset(String src, String dest)
  {
    Airline subset = new Airline(name);
    for (Flight flight : getFlights())
    {
      if (src.equals(flight.getSource()) && dest.equals(flight.getDestination()))
      {
        subset.addFlight(flight);
      }
    }
    return subset;
  }
